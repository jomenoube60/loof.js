...
class Ball {
    constructor(menu) {
        ...
        shape.set_m_radius(PLR_RADIUS / 2 / PHY_SCALE);
        fixdef.set_shape(shape);
        fixdef.set_restitution(0.95);
    ...
    set_position(x, y) {
        ...
        temp.Set(x / PHY_SCALE, -y / PHY_SCALE)
...
class Game {
    ...
    restart() {
        ...
        this.ball.set_position(
            (level.starting_points[0][0] + level.starting_points[1][0]) / 2,
            (level.starting_points[0][1] + level.starting_points[1][1]) / 2
        )
        this.enemies.forEach(dude => dude.set_position(...level.starting_points[1], 10, 3.1415 / 2))
    }
    update(dt, delta) {
        // TODO: detect ball out-of-field & place it to center
        let [bx, by] = [this.ball.pos.get_x(), this.ball.pos.get_y()]
        let r = (10+PLR_RADIUS)/PHY_SCALE
        for (let dude of [this.player, ...this.enemies]) {
            if (Math.sqrt(Math.pow(bx - dude.pos.get_x(), 2) + Math.pow(by - dude.pos.get_y(), 2)) <= 3) {
                dude.take_ball(this.ball)
            }
        }
        // direction keys
    ...
    draw(dt, delta) {
        ...
        let max_x = PHY_SCALE * (buddies.reduce((val, dude) => Math.max(val, dude.pos.get_x()), 0))
        let min_x = PHY_SCALE * (buddies.reduce((val, dude) => Math.min(val, dude.pos.get_x()), app.display.width))
        let max_y = PHY_SCALE * (-buddies.reduce((val, dude) => Math.min(val, dude.pos.get_y()), 0))
        let min_y = PHY_SCALE * (-buddies.reduce((val, dude) => Math.max(val, dude.pos.get_y()), -app.display.height))
        // y is inverted
