...
class Game {
    ...
    update(dt, delta) {
        ...
        for (let dude of [this.player, ...this.enemies]) {
            if (Math.sqrt(Math.pow(bx - dude.pos.get_x(), 2) + Math.pow(by - dude.pos.get_y(), 2)) <= r) {
                // if free ball take it, else 30% change to get it
                if (!!!this.ball.owned || Math.random() < 0.3) {
