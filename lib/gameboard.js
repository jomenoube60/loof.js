...
class Ball {
    constructor(menu) {
        ...
        player_by_fixture[body.CreateFixture(fixdef).ptr] = this
...
class Game {
    ...
    update(dt, delta) {
        ...
        // contact handling
        for (let contact = this.world.GetContactList(); !!contact.ptr; contact = contact.GetNext()) {
            if (contact.IsTouching()) {
                let fixA = contact.GetFixtureA()
                let fixB = contact.GetFixtureB()
                let plrA = player_by_fixture[fixA.ptr]
                let plrB = player_by_fixture[fixB.ptr]
