...
class Game {
    ...
    update(e) {
        ...
        if(this.key.pressed(e, 'UP')) {
            this.bodies[0].ApplyForceToCenter(new Box2D.b2Vec2(0, PLR_FORCE))
            propagate = false
        }
    ...
    draw(dt, delta) {
        ...
        this.ctx.drawImage(this.dude_img,
            PHY_SCALE * (pos.get_x() - PLR_RADIUS) * scale_ratio,
            -PHY_SCALE * scale_ratio * (pos.get_y() + PLR_RADIUS),
            this.dude_img.width*scale_ratio,
             this.dude_img.height*scale_ratio); // WARNING: y coordinates inverted
    }
}
