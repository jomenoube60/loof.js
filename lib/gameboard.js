...
class Game {
    ...
    update(dt, delta) {
        ...
        for (let contact = this.world.getContactList(); !!contact; contact = contact.getNext()) {
            if (contact.isTouching()) {
                ...
                if (!!ball) {
                    if (plr != ball.last_owner || ball.lastshot + 50 < dt) { // shooting trick
                    ...
                    if (plrA.actions.pusher.running)
                        plrB.push()
                    if (dt < plrB.actions.pusher.running)
                        plrA.push()
                }
    ...
    draw(dt, delta) {
        ...
        if (this.goalmarked.running) {
            app.display.blit(this.goals_img[this.goalmarked.where], 0, 0, {fixratio:true, scale: 1+(Math.random()/10), opacity: 0.5 + Math.random()})
        }
