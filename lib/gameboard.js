...
class Game {
    ...
    update(e) {
        ...
        if(this.key.pressed(e, 'UP')) {
            this.player.body.ApplyLinearImpulse(this.player.body.GetWorldVector(new Box2D.b2Vec2(0, PLR_FORCE)), this.player.body.GetPosition(), true);
        } else if(this.key.pressed(e, 'DOWN')) {
            this.player.body.ApplyLinearImpulse(this.player.body.GetWorldVector(new Box2D.b2Vec2(0, -PLR_FORCE / 1.25)), this.player.body.GetPosition(), true);
            propagate = false
        } else if(this.key.pressed(e, 'RIGHT')) {
            this.player.body.SetTransform(this.player.body.GetPosition(), this.player.body.GetAngle() - 0.75);
            propagate = false
