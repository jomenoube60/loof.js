...
class Player {
    constructor(menu, image) {
        ...
        this.ball = false
        this.actions = {
            'boost': new TimedAction({duration:20}),
            'shoot': new TimedAction({duration:10}),
            'pusher':new TimedAction({duration:10}),
        }
    }
    update(dt, delta) {
    ...
    targets(coords, delta) {
        ...
        var angle_correction = (this.pos.x > coords.x)? Math.PI:0
        var myangle = (Math.PI / 2 + (this.body.getAngle() - angle_correction)) % (2 * Math.PI);
