...
class Player {
    ...
    destroy() {
        ...
        this.image = null
        this.world = null
        for (let ptr in player_by_fixture) {
            if (player_by_fixture[ptr] == this) {
                delete player_by_fixture[ptr]
            }
        }
    }
    ...
    push(dt) {
        if (!!this.ball) {
            ...
            this.ball.body.ApplyLinearImpulse(this.ball.body.GetWorldVector(new Box2D.b2Vec2(0, 3 * PLR_FORCE)), this.ball.body.GetPosition(), true);
            this.ball = false
        } else {
