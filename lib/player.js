...
class Player {
    ...
    push(dt) {
        ...
        if (!!this.ball) {
            let force = PLR_FORCE * 2
            this.ball.owned = false
            let temp = new Box2D.b2Vec2(0.0, 0.0)
            let angle = this.body.GetAngle()
            let [x, y] = [4 * Math.sin(-angle), 4 * Math.cos(-angle)]
            ...
            this.ball.body.ApplyLinearImpulse(this.ball.body.GetWorldVector(new Box2D.b2Vec2(0, force)), this.ball.body.GetPosition(), true);
            this.ball = false
        } else {
