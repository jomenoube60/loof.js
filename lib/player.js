...
class Player {
    ...
    targets(coords, coord2 ,delta) {
      var sync = false
      var distance = this.distance(coords)
      var x = coords[0]-this.body.GetPosition().get_x()
      var y = coords[1]-this.body.GetPosition().get_y()
      var angle =   Math.acos(x / distance ) % (2*Math.PI)
      if (this.body.GetAngle() % (2*Math.PI) > angle * (1-delta) && this.body.GetAngle() % (2*Math.PI) < angle * (1+ delta)) {
        sync = true

      }
      console.log(sync , this.body.GetAngle() % (2*Math.PI));
      return angle
