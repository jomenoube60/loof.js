class PlayerAI {
    ...
    manage(player, dt) {
        ...
        if (player.ball) {
            ...
            } else if (infos.lastmode == null || infos.lastmode_ts > mode_duration then) {
                infos.lastmode_ts = 0
                if (this.toball_cnt > 0 && (Math.random() < 0.5 or this.toball_cnt == 0)) {
                    infos.mode = 'agressive'--make one agressive, then random
                    this.agressive_cnt = this.agressive_cnt + 1
                } else {
                    infos.mode = 'toball'--
                        if one agressive then one here, then random
