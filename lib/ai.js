class PlayerAI {
    ...
    manage(player, dt, delta) {
        ...
        if (infos.mode == "aggressive") {
           let random_player = app.menus[0].enemies[Math.floor(Math.random(1, app.config.DUDES))]
          //  let random_player = app.menus[0].enemies[0]
            var dist = player.distance_from(random_player.pos)
            let targets = player.targets(random_player.pos)
            console.log(targets);
            if (dist < 10 && !targets[0]) {
              if (targets[1] > 0) {
                player.left(0.1)
              } else {
                player.right(0.1)
              }
            } else if (dist > 10 && targets[0]) {
